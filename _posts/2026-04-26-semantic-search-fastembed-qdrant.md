...
Semantic search takes a more intelligent approach. Instead of relying on exact word matches, it represents both queries and documents as vectors that capture meaning. It effectively asks, ***"Is this document conceptually similar to what the user is looking for?"*** By understanding context, synonyms, and intent, it can connect ideas that aren’t phrased the same way ([1][1], [2][2], [3][3]). In this sense, semantic search focuses on interpreting meaning rather than matching words, enabling it to deliver results that are more accurate and relevant.
...
Semantic search isn’t always the clear winner. In fact, the traditional keyword-based algorithm BM25, which doesn’t rely on training data and is purely statistical, still outperforms dense retrieval on 9 of the 18 datasets in the BEIR benchmark under zero-shot conditions ([4][4]). This highlights that classical methods remain valuable, especially for precise matches such as codes, IDs, or jargon-heavy queries.
...
- Dense Embeddings: These produce a single, compact vector (e.g., 384, 768, or 1536 dimensions) that represents the overall meaning of the input as a point in a high-dimensional space ([5][5]).
- Sparse Embeddings: These are high-dimensional vectors where most values are zero ([5][5], [6][6]). They excel at capturing specific keywords and exact term matching within a semantic space, making them ideal for hybrid search ([7][7]).
- Multimodal Embeddings: These models can process different types of data, such as text, images, audio, or video, and map them into a shared vector space, enabling cross-modal understanding ([8][8]).
- Multi-Vector Embeddings (Late Interaction): Models like ColBERT generate multiple vectors per document, allowing for token-level matching and finer-grained retrieval ([9][9]).

---

## 🔍 Similarity Metrics

Once we have our query vector and a database of document vectors, we need a way to find which ones are nearest to each other. This is where similarity metrics come in. Qdrant supports four main [distance metrics](https://www.mintlify.com/qdrant/qdrant/concepts/distance-metrics) out of the box ([10][10]).
...
The old debate of "keyword vs. semantic" is dead. At [ECIR 2026](https://ecir2026.eu/), *every* production search system presented used  **hybrid retrieval**, combining keyword methods like BM25 with dense semantic vectors ([12][12], [13][13]). The question has shifted from *"should we?"* to *"how efficiently and at what cost?"*.

### 🤹 Multi-Vector & Late Interaction Models

Multi-vector models like **ColBERT** are gaining attention ([14][14]). They create multiple embeddings per document to enable token-level matching, providing a nuanced middle ground between sparse and dense retrieval.

### ✨ Multimodal Search

The era of search across different media is here. Libraries include [**Gemini Embedding 2**](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/models/gemini/embedding-2), **LanguageBind** ([15][15]), and [**UForm**](https://github.com/unum-cloud/uform) can all map text, images, and video into a shared vector space, enabling cross-modal retrieval.
...
| -------------------------------- | ------------------------------------------- | --------------------------------------------------------------------- |
| **Qdrant** ([16][16])      | High-scale, high-performance vector search  | Hybrid search, multitenancy, quantization, Web UI dashboard, gRPC     |
| **Meilisearch** ([17][17]) | Drop-in replacement for Algolia-like search | Hybrid semantic & keyword search, multi-format support, Docker-native |
| **Marqo** ([18][18])       | Multimodal search out-of-the-box            | LanguageBind models for video, audio, image, text search              |
| **txtai** ([19][19])       | All-in-one embeddings database              | SQL support, graph networks, RAG capabilities, multimodal indexing    |
| **Vespa** ([20][20])       | Large-scale production search               | Hybrid retrieval, ONNX runtime, advanced ranking                      |
...
The fastest way to get Qdrant running is with Docker ([21][21]). If you haven't already, run Qdrant locally, pull the official image and run it with persistent storage:
...
First, install Qdrant with its FastEmbed extension([22][22]), along with data handling tools:
...
We'll use a real-world dataset: **"Enriched Tourism Dataset Paris (POIs)"** available on Mendeley Data ([23][23]). This dataset contains tourist reviews, destinations, and other valuable travel information well-suited for semantic search applications. It is derived from the Paris subset of the Tourpedia dataset, which focuses on points of interest (POIs) classified as attractions (available at [http://tour-pedia.org/download/paris-attraction.csv](http://tour-pedia.org/download/paris-attraction.csv)).
...
Now we know that we’re working with English text, the next step is to select an appropriate embedding model to convert this content into vector representations ([24][24]).
...
2. Winks, Emily. (March 04, 2026). **Semantic Search vs. Keyword Search: The Comparison Nobody Finishes [2026]**.
3. Laurel, Hannah. (December 15, 2025). **Semantic Search vs. Keyword Search: What’s the Difference?**.
...
5. Sugimori, Dai. (2025). **Understanding sparse vector embeddings with trained ML models**.
6. OpenSearch. (2026) **Neural sparse search**.
7. Bhagdev, R., Chapman, S., Ciravegna, F., Lanfranchi, V., & Petrelli, D. (2008, June). **Hybrid search: Effectively combining keywords and semantic searches**. In European semantic web conference (pp. 554-568). Berlin, Heidelberg: Springer Berlin Heidelberg.
8. Alibaba Cloud Model Studio. (2026). **Text and Multimodal Embedding**.
9. Qdrant. (2026). **Multivectors for Late Interaction Models**.
10. Qdrant. (2026). **Distance Metrics**.
11. Hugging Face. (April 25, 2026). **Embedding Leaderboard: MTEB(Multilingual, v2)**.
12. DataTalksClub. (2026). **LLM Zoomcamp**.
13. Algolia. (April 23, 2026) **ECIR 2026: The Evaluation Renaissance**.
14. Patel, R. N., & Dutta, S. (2026). **ColBERT-Att: Late-Interaction Meets Attention for Enhanced Retrieval**. arXiv preprint arXiv:2603.25248.
...
16. Qdrant. (2026). **Qdrant Documentation**.
17. Meilisearch. (2026). **Meilisearch**.
18. Marqo AI. (July 18, 2025). **Benchmarking Models for Multi-modal Search**.
19. txtai. **txtai**. (2026).
20. Vespa. **Vespa**. (2026).
21. Qdrant. (2026). **How to Get Started with Qdrant Locally**.
22. Qdrant. **FastEmbed**. (2026).
23. Hermoso, Ramon , Ilarri, Sergio, and Trillo-Lado, Raquel. (October 30, 2024). **Enriched Tourism Dataset Paris**. Mendeley Data, V1, doi: 10.17632/vh4g4g2322.1.
24. DataTalksClub. (2026). **Vector Search with Qdrant**.
...
[4]: https://arxiv.org/abs/2104.08663
[5]: https://www.elastic.co/search-labs/jp/blog/sparse-vector-embedding
[6]: https://docs.opensearch.org/2.14/search-plugins/neural-sparse-search/
[7]: https://link.springer.com/chapter/10.1007/978-3-540-68234-9_41
[8]: https://www.alibabacloud.com/help/en/model-studio/embedding
[9]: https://qdrant.tech/course/essentials/day-5/colbert-multivectors/
[10]: https://www.mintlify.com/qdrant/qdrant/concepts/distance-metrics
[11]: https://huggingface.co/spaces/mteb/leaderboard
[12]: https://github.com/DataTalksClub/llm-zoomcamp
[13]: https://www.algolia.com/blog/engineering/ecir-2026-recap
[14]: https://arxiv.org/html/2603.25248v1
[15]: https://arxiv.org/abs/2310.01852
[16]: https://qdrant.tech/documentation/
[17]: https://github.com/meilisearch/meilisearch
[18]: https://www.marqo.ai/blog/benchmarking-models-for-multimodal-search
[19]: https://github.com/neuml/txtai
[20]: https://github.com/vespa-engine/vespa
[21]: https://qdrant.tech/documentation/quickstart/
[22]: https://qdrant.github.io/fastembed/?trk=public_post_comment-text
[23]: https://data.mendeley.com/datasets/vh4g4g2322/1
[24]: https://github.com/DataTalksClub/llm-zoomcamp/blob/main/02-vector-search/sematic_search.ipynb
---
